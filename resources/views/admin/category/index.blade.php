				...
				<div class="container-fluid">
					<div class="row mb-2">
						...
						<div class="col-lg-6 col-md-6 col-sm-6 col-xs-12 right-align">
							...
							<a href="{!! route('admin.add.category') !!}" class="btn btn-primary"><i class="fa fa-plus"></i>  &nbsp; Add</a>
							{{-- @endif --}}
						</div>
					</div>
				</div><!-- /.container-fluid -->
											...
											<tbody>
												...
												@foreach($category as $item)
													<tr>
														<td>{!! ++$i !!}</td>
														<td>{!! $item->name !!}</td>
														<td><img src="{{asset('public/category_image/'.$item->image)}}" width="60" height="60" /></td>
														<td>{!! Carbon\Carbon::parse($item->created_at)->format(config('app.date_time_format')) !!}</td>
														<td>
															@if(empty($item->deleted_at))

																{{-- @if(Helper::checkPermission('edit-category')) --}}
																<a href="{!! route('admin.edit.category', $item->id) !!}"  title="Edit" class="btn-sm btn btn-primary"><i class="fa fa-edit"></i>  </a>
																{{-- @endif --}}
	    ...
	    <div class="modal-dialog">
	      <div class="modal-content">
	        ...
	        <div class="modal-footer">
	          ...
	          <a href="{{ url('/admin/delete-category/11') }}" id="delete-category" class="btn btn-primary">Delete</a>
	        </div>
	      </div>
	      <!-- /.modal-content -->
	    </div>
	...
	<div class="modal fade" id="modal-category-restore">
        <div class="modal-dialog">
          <div class="modal-content">
            ...
            <div class="modal-body">
              <p>Are you sure? You want to Restore category.</p>
            </div>
            <div class="modal-footer">
              <button type="button" class="btn btn-danger" data-dismiss="modal">Close</button>
              <a href="{{ url('/admin/restore-category/11') }}" id="restore-category" class="btn btn-primary">Restore</a>
            </div>
          </div>
          <!-- /.modal-content -->
        </div>
