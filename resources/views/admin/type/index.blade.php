...
<body class="hold-transition sidebar-mini layout-fixed">
				...
				<div class="container-fluid">
					<div class="row mb-2">
						...
						<div class="col-lg-6 col-md-6 col-sm-6 col-xs-12 right-align">
							...
							<a href="{!! route('admin.add.type') !!}" class="btn btn-primary"><i class="fa fa-plus"></i>  &nbsp; Add</a>
											...
											<tbody>
												...
												@foreach($type as $item)
													<tr>
														<td>{!! ++$i !!}</td>
														<td>{!! $item->name !!}</td>
														<td><img src="{{asset('public/type_image/'.$item->image)}}" width="60" height="60" /></td>
														<td>{!! Carbon\Carbon::parse($item->created_at)->format(config('app.date_time_format')) !!}</td>
														<td>
															@if(empty($item->deleted_at))

																<a href="{!! route('admin.edit.type', $item->id) !!}"  title="Edit" class="btn-sm btn btn-primary"><i class="fa fa-edit"></i>  </a>
	    ...
	    <div class="modal-dialog">
	      <div class="modal-content">
	        ...
	        <div class="modal-footer">
	          ...
	          <a href="{{ url('/admin/delete-type/11') }}" id="delete-type" class="btn btn-primary">Delete</a>
	        </div>
	      </div>
	      <!-- /.modal-content -->
	    </div>
	...
	<div class="modal fade" id="modal-type-restore">
        <div class="modal-dialog">
          <div class="modal-content">
            <div class="modal-header">
              <h4 class="modal-title">Restore Type</h4>
              <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true">&times;</span>
              </button>
            </div>
            <div class="modal-body">
              <p>Are you sure? You want to Restore Type.</p>
            </div>
            <div class="modal-footer">
              <button type="button" class="btn btn-danger" data-dismiss="modal">Close</button>
              <a href="{{ url('/admin/restore-type/11') }}" id="restore-type" class="btn btn-primary">Restore</a>
            </div>
          </div>
          <!-- /.modal-content -->
        </div>
